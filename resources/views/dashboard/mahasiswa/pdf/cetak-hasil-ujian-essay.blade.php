...
@section('content')
    ...
    <table align="center">
        ...
        <tr class="satu">
            <td>Mulai Ujian</td>
            <td>:
                {{ Carbon\Carbon::parse($mahasiswa->started_at)->format('H:i:s') }} -
                {{ Carbon\Carbon::parse($mahasiswa->ended_at)->format('H:i:s') }} WIB
            </td>
        </tr>
        ...
        <tr>
            <td>Jumlah Salah</td>
            <td>:
                {{ $mahasiswa->ujianHasil->where('status', '0')->where('jawaban', '!=', null)->count() }}</td>
        </tr>
        ...
        <tbody>
            @foreach ($ujianHasil as $ujian)
                ...
                <tr>
                    ...
                    <td>
                        @if ($ujian->komentar_dosen == null)
                            -
                        @else
                            {{ $ujian->komentar_dosen }}
                        @endif
                    </td>
                </tr>
            @endforeach
